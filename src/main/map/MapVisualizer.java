...
public class MapVisualizer {
    ...
    private String drawObject(Vector2d currentPosition) {
        ...
        if (this.map.isOccupied(currentPosition)) {
            ...
            if (object instanceof Grass) result = ((Grass) object).toString();
            else if (object instanceof ArrayList) {
                ArrayList<Animal> arraylistObject = ((ArrayList) object);
                if (arraylistObject.size() == 0) result = EMPTY_CELL;
                if (arraylistObject.size() == 1) result = (arraylistObject.get(0).toString());
                else {
                    Animal strongest = null;
                    for (int i = 0; i < arraylistObject.size(); i++) {
                        if (strongest == null || arraylistObject.get(i).getEnergy() > strongest.getEnergy()) {
                            strongest = arraylistObject.get(i);
                        }
                    }
